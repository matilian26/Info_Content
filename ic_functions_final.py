                ...
                if len(tempdictionary) > 0:
                    if form is 'PWM':
                        tempdictionary = np.vstack((tempdictionary, \
                                                    np.array([int(val)+pscount for val,pscount in \
                    ...
                    if form is 'PWM':
                        tempdictionary = np.array([int(val)+pscount for val,pscount in \
                                            zip(line.split(),weighted_pscount)])
                    elif form is 'list_num':
                        tempdictionary = [float(x) for x in line.strip().split(sep)]
                    elif form is 'list_str':
...
def scoreCalc(sequence_dictionary, TF_list, PWM_dictionary, score_data, perc=25):
    ...
    with open('patser_output.txt','w') as f:
        for TF in TF_list:
            for seq_name, sequence in sequence_dictionary.iteritems():
...
def createSeqIN(dictionary,filter_string,regex,filepath,filename,write,FastA):
    ...
    if filter_string != 0:
        filtered_dict = {key:value for (key,value) in dictionary.iteritems() \
...
def runPatser(filepath,origFile,TF_list,print_restrict='0',percentile=75,aligned=0):
...
def parsePatser(filepath,TF_list,patserFile,lnp,category,percentile=75,input_extension='.txt'):
    ...
    TFs_skipped = []
    output = pd.DataFrame(columns=['enhancer','tf','position','score','lnp'])
    ...
    with open('%s/patserAnalysis/%s_enhancers_patser_output%s'%(filepath,category,extension),'w') as f:
        for tf in TF_list:
    ...
    output = pd.read_csv('%s/patserAnalysis/%s_enhancers_patser_output%s' %(filepath,category,extension),sep='\t')
    output.columns = ['enhancer','tf','position','score','lnp']
    output[['score','lnp']] = output[['score','lnp']].astype(float)
    # write patser output to file
#   output.to_csv('patserAnalysis/%s_enhancers_patser_output%s'%(category,extension),sep='\t',index=False)
    return output
...
def noDuplicates(output,category,PWM_offset,filepath):
    ...
    #         print tf
            for pos,idx in zip(temp_tf.position,temp_tf.index.tolist()):
                for partner,offset in PWM_offset[tf].iteritems():
                    if 'C' in pos:
